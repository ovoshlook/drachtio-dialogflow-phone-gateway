...
let welcomeEvent = config.has('dialogflow.events.welcome') ? config.get('dialogflow.events.welcome') : '';
...
class CallSession extends Emitter {
  ...
  async exec() {
    try {
      ...
      welcomeEvent = this.getCalleeInfo(this.req.msg.uri);
      if (!welcomeEvent) {
          return;
      }
      this.logger.info("Got Welcome Event as intent:",welcomeEvent)
